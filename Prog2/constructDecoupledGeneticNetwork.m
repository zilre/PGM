...
for i=1:numPeople
    if sum(pedigree.parents(i,:)) == 0 % indiv is founder
     ...
     else %indiv is non-founder
        parents= [i,pedigree.parents(i,:)];
        factorList(i)=childCopyGivenParentalsFactor(numAlleles, i, parents(2), parents(2)+numPeople);
        factorList(i+numPeople)=childCopyGivenParentalsFactor(numAlleles, i+numPeople, parents(3), parents(3)+numPeople);
    end
    phenoindex=(2*numPeople)+i;
    factorList(phenoindex)=phenotypeGivenCopiesFactor(alphaList, numAlleles, i, numPeople+i, phenoindex);
end
