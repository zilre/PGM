...
ngenos=numbergenotypes(numAlleles);
...
genotypeFactor.card= [ ngenos, ngenos, ngenos ];
genotypeFactor.var= [ genotypeVarChild, genotypeVarParentOne, genotypeVarParentTwo ];
genotypeFactor.val = zeros(1, prod(genotypeFactor.card));
% Replace the zeros in genotypeFactor.val with the correct values.

for z=1:prod(genotypeFactor.card )
    %assignments=[ child, p1, p2 ]
    assignments=IndexToAssignment(z, genotypeFactor.card);
    childgeno=assignments(1);       

    parent1gametes = genotypesToAlleles(assignments(2),:);
    parent2gametes = genotypesToAlleles(assignments(3),:);
    punnet=[];
    k=1;
    for i=1:length(parent1gametes)
        for j=1:length(parent2gametes);
            gamete=allelesToGenotypes( parent1gametes(i), parent2gametes(j) );
            punnet(k)=gamete;
            k=k+1;
        end
    end;
    %punnet
    punnet_freq=histc(punnet, [1:ngenos])/4;
    %punnet_freq(childgeno)
    genotypeFactor.val(z)=punnet_freq(childgeno);
