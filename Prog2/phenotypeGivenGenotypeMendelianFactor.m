...
[nrows, ncols]=size(assignments);

for i=1:nrows
    geno_pheno=assignments(i,:);
    %display(geno_pheno);
    
    if isDominant==1 % is it dominant model
       if geno_pheno(1) == 1 % affected phenotype
           if geno_pheno(2) == 1 | geno_pheno(2) == 2
               phenotypeFactor.val(i)=1;
           else
               phenotypeFactor.val(i)=0;
           end %close geno_pheno(2) == 1 or 2
       else
           if geno_pheno(2) == 3
            phenotypeFactor.val(i)=1;
           end
    ...
    end %close isDominant==1

    if isDominant==0 %its recessive
        if geno_pheno(1) == 1 %affected phenotype
            if geno_pheno(2) == 3
                phenotypeFactor.val(i)=1;
            end
