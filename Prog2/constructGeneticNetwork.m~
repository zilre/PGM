...
for i=1:numPeople
    if sum(pedigree.parents(i,:)) == 0 % indiv is founder
        ...
        factorList(i)=genotypeGivenParentsGenotypesFactor(numAlleles, i, parents(2), parents(3) );
    end
    factorList(i+numPeople)=phenotypeGivenGenotypeFactor(alphaList, i, i+numPeople);
    %phenotypeGivenGenotypeFactor(alphaList, numAlleles, geneCopyVarOne, geneCopyVarTwo, phenotypeVar);
end
