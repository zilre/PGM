...
uvar=[];
if n==2
    uvar = union( F(1).var, F(2).var );
    
  else
    uvar=union(F(n).var, F(n-1).var );
    i=n-2;
    for j=fliplr(2:i)
        uvar=union(uvar, F(j).var );
    end
v=setdiff(uvar,V);

jointE= ObserveEvidence( ComputeJointDistribution ( F ), E);
jointE.val=jointE.val/sum(jointE.val);
jointE;

M=FactorMarginalization(jointE, v);
